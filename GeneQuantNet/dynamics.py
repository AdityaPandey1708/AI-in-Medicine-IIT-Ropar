...
def get_masks_from_offset(p, iscell=None, rpad=20, threshold=0.0):
...
def remove_bad_flow_masks(masks, flows, threshold=99999999999, use_gpu=False, device=None):
...
def get_masks(p, iscell=None, rpad=20, threshold=999999999):
    """ create masks using pixel convergence after running dynamics
...
def compute_masks(offsetmap, confimap, p=None, niter=200, 
                   ...
                   flow_threshold=0, interp=True, do_3D=False, 
                   min_size=-1, resize=None, 
                   use_gpu=False,device=None):
    """ compute masks using dynamics from offsetmap, confimap """
    
    cp_mask = confimap > -0.1#confidence_threshold 
...
def get_mask(center_coord, p, cp_mask, flow_threshold=0):
...
def compute_masks_from_offset(offsetmap, centermap, confimap, p=None, 
                   ...
                   flow_threshold=0, interp=True, do_3D=False, 
                   min_size= -1 , resize=None,   #min_size here was 300 
                   use_gpu=False,device=None):
    """ 
    ...
    cp_mask = confimap > -1 #confidence_threshold 
